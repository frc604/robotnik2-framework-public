...
public class Robot extends TimedRobot {
  ...
  private void updateModuleList() {
    table
        .getEntry("moduleList")
        .setString(modules.stream().map(Module::getName).collect(Collectors.joining(",")));
  }
  ...
  private void loopInit(String name, Coordinator mode) {
    ...
    for (Module module : modules) {
      Reliability.swallowThrowables(
          module::begin, "Error in begin() of module " + module.getName());
    }
    ...
    for (Pair<String, Coordinator> system : systems) {
      Reliability.swallowThrowables(
          system.getValue()::start, "Error starting system " + system.getKey());
    }
  }
  ...
  private void loopExecute(String name, Coordinator mode) {
    ...
    for (Pair<String, Coordinator> system : systems) {
      Reliability.swallowThrowables(
          system.getValue()::execute, "Error executing system " + system.getKey());
    }
  ...
  private void loopEnd(String name, Coordinator mode) {
    ...
    for (Pair<String, Coordinator> system : systems) {
      Reliability.swallowThrowables(
          system.getValue()::stop, "Error stopping system " + system.getKey());
    }
