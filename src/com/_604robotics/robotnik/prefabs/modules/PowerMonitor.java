...
public class PowerMonitor extends Module {
  ...
  public static double[] currentLimit = {
    40, 40, 40, 40, 20, 20, 20, 20, 20, 20, 20, 20, 40, 40, 40, 40
  };
  ...
  public PowerMonitor(int PDPPortID, int compressorID) {
    ...
    totalCurrent =
        addOutput(
            "Total Current",
            () -> /*panel.getTotalCurrent()+compressor.getCompressorCurrent()*/ -1.0);
    ...
    for (int port = 0; port < NUM_CURRENT_PORTS; port++) {
      ...
      Output<Double> tempOutput =
          addOutput("Port " + port + " Current", () -> /*panel.getCurrent(proxy)*/ -1.0);
    ...
    for (int i = 0; i < NUM_CURRENT_PORTS; i++) {
      if (timerArray[i].isRunning()) {
        if ((timerArray[i].get() % 0.5) < 0.05 && (timerArray[i].get() > 0.44)) {
          theLogger.log(
              "WARN", "Excess current of " + currents[i].get() + " through PDP Port " + i + "!");
    ...
    for (int i = 0; i < NUM_CURRENT_PORTS; i++) {
      if (currents[i].get() > currentLimit[i]) {
        ...
        if (pulseArray[i].isFallingEdge() && timerArray[i].get() > 0.4) {
          theLogger.log(
              "WARN",
              "Excess current ended through PDP Port "
                  + i
                  + " after "
                  + timerArray[i].get()
                  + " seconds.");
